...
export const PurchasesPage = () => {
  const { fetchPurchases, fetchAssets, purchases, assets, loading, addPurchase } = useAPI();
  ...
  const loadData = async () => {
    ...
    const purchasesResult = await fetchPurchases();
    if (!assetsResult.success || !purchasesResult.success) {
      setError(assetsResult.error || purchasesResult.error);
    }
  };
  ...
  const handleAddPurchase = async (e) => {
    ...
    if (result.success) {
      ...
      setFormData({ assetId: '', quantity: '', base: '' });
      setShowForm(false);
    } else {
      ...
      <div className="space-y-6">
        ...
        {showForm && (
          <div className="card border-l-4 border-l-blue-600">
            <h2 className="section-title mb-4">Record Purchase</h2>
        ...
        {purchases.length > 0 ? (
                ...
                {purchases.map((purchase, idx) => (
                  <tr key={idx} className="table-row border-b border-gray-200 hover:bg-gray-50">
                    <td className="table-cell text-gray-800 font-medium">
                      {purchase.asset?.assetName || 'N/A'}
                    </td>
                    <td className="table-cell text-center font-semibold text-gray-800">
                      {purchase.quantity}
                    </td>
                    <td className="table-cell text-gray-800">{purchase.base}</td>
          ...
          <NoData message="No purchases recorded yet" />
        )}
      </div>
    </Layout>
  );
